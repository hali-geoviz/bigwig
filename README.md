...
6. Next section is **Well log and parameter comparison**. Here there are two different comaprisons: 
   - "Comparing two parameters from same well" (1)
   - "Comparing five wells with two parameters each" (2)
...
Python modules we've experimented with, though did not end up in our finalized code:
   ...
   - Welly. A package with main purpose to graph well logs. Experimenting with it, produce some simple yet informative well logs. Ideal to provide general inforamtion about the file in regards to where the well is located, type of measurments in the file and depth of mesurments. Very good for geophysicist when doing simple well log plotting and this module is created for intrepertating well logs. However, we didn't feel like it provided any additional valuable information compared to the graphs we created with the module hvplot. And since we were already deep into hvplot, we found that it would be  unnesessary and time-consuming to find out how elements from each of these packages could be interactively connected in a dynamic map. Recommend to go a webpage called Slack (and the group community: welly) to get in touch with people that is developing this software.
