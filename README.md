...
6. Next section is **Well log and parameter comparison**. This part of the visualization is directed towards the ascpect of the thought of work flow of analysis and comparing huge amounts of well log data to then picking out a few wells to go more into depth to. Here there are two different comaprisons: 

- "Comparing two parameters from same well"

   - Apart from the very obious plot titles the first one is set to specifically look at only one well's parameters and graphs two parameters against each other. the widget "well" will allow you to select what well you want to view where the drop down menu is a list of all wells in the dataframe. the second widget labeled "y_axis" is a text box that is pre-set to display the depth parameter of the chosen las file, but can manually be changed to what ever parameter you want, given that you know the name of it. third and fourth widgets are two drop down menus for what parameter you want to compare against each other on the x-axis. For "x_axis" and "x_axis2" you can choose between all the parameters that the las files in the dataframe contain.
...
- "Comparing five wells with two parameters each"
...
Future improvements:
   ...
   - The data manipulation and analysis library Dask utilizes split CPU core computation and run larger sets of data in a much more efficient fashion than the more popular dataframe library pandas making it the favorable choise when handling big data. This runbook should incorporate the use of dask much more than it does, but as the data used so far as been caught in the grey zone between big data sized data sets and regular sized data sets, it was hard to really see a difference in performance between using Dask for holding the dataframe compared to pandas.
   
   - Panel is a library that further builds on the hvplot and holoviews data analysis and visualization libraries that have been heavily used in this notebook, and makes it possible to create very modular and customizable locally hosted web browser run dashboards in a seperate tab from the jupyter notebook itself. It would have been ideal to make all the plots in the notebook file support some sort of panel powered dashboard projected up into a seperate web browser tab making the code even easier for people with little programming experience use the notebook.
...
Disclaimer:
   ...
   - The guidelines for this project were to stay fully open source meaning that the team did not have access to any of Equinor's own well log files and as such the creation and use of artificially created well log .las files were mainly used during the planning and creation of the big data visualization.
